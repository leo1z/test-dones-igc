...
*   [x] **Ajustes de Maquetación del Wizard del Test (`index.html`, `styles.css`, `app.js`):**
    *   **Visibilidad de Instrucción Honesta:** Restringida la tarjeta de instrucción únicamente a la **Parte 1** y **Parte 5** (`sceneId === 1 || sceneId === 5`), ocultándola en el resto de partes para aligerar la vista.
    *   **Rediseño y Proporciones de Banners Hito:** Removido el sufijo `(PARTE X)` de los títulos de banner. Reducido el tamaño de imagen a `65×65 px`, ajustado el relleno a `14px 20px` y acortado el texto descriptivo a una línea concisa en partes hito (Parte 2: *¡BUEN COMIENZO!*, Parte 5: *¡MITAD DEL CAMINO!*, Parte 9: *¡YA CASI TERMINAS!*).
    *   **Barra de Guía de Dones en TODAS las Partes (1 a 10):** Creada una fila informativa independiente que aparece en todas las partes con el texto: *“Conocer los dones te dará más claridad.”* y enlace interactivo `Aprender más de los Dones →` para desplegar la consulta rápida sin desbordamientos de texto.
