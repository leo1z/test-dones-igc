...
*   [x] **Apertura de Grupos de Crecimiento en Pestaña Independiente (`index.html`, `app.js`):** Removida la intercepción modal interna (iframe). Ahora al hacer clic en *"Conectar con un Grupo de Crecimiento"* se abre directamente la plataforma oficial (`https://igcteg.org/grupos/`) en una **pestaña independiente del navegador** (`target="_blank" rel="noopener noreferrer"`).
