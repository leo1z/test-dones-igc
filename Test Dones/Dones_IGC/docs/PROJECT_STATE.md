...
*   [x] **Rediseño de UI de Filtros Autónomos (`admin.html`):** Rediseñada la barra `.chart-controls-bar` con una disposición de **CSS Grid auto-fit** (`minmax(210px, 1fr)`) y etiquetas verticales sobre los selectores desplegables (`flex-direction: column`). Esto elimina cualquier solapamiento de texto, truncamientos feos u desbordamientos laterales en pantallas móviles y de escritorio.
