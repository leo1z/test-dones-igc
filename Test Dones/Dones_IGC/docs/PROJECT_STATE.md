...
*   [x] **Mapa Coroplético Oficial de Honduras con Apache ECharts (`admin.html`):** Reemplazada la ilustración SVG manual por una integración con **Apache ECharts** y la capa GeoJSON oficial de los 18 departamentos de Honduras (`SEFINHN-UIT/hn-geo`). Cada departamento se pinta según el volumen de evaluaciones registradas con paleta visual escalar (`#e2e8f0` a `#0f0f31`) y tooltips interactivos al pasar el cursor (nombre del departamento, total de encuestas y porcentaje).
*   [x] **Tarjeta Top 3 Dones por Ubicación (`admin.html`):** Ampliada la tarjeta #5 para mostrar los **Top 1, Top 2 y Top 3 dones principales** por cada ciudad seleccionada en el menú desplegable.
*   [x] **Filtros Autónomos Completos de Fecha y Ubicación en Análisis de 15 Dones:** Incorporados controles internos independientes de **Rango de Fecha** (`cross-filter-date`) y **Ubicación** (`cross-filter-location`) que se combinan acumulativamente con *Asistencia a Grupo* y *Reflejo de Servicio*.
