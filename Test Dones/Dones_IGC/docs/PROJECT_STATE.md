...
*   [x] **Apertura On-Site de 'Conectar con un Grupo de Crecimiento' (`index.html`, `app.js`):** Reemplazada la redirección externa a `igcteg.org/grupos/` por un modal interactivo superpuesto dentro de la misma aplicación (`#modal-growth-group-onsite`) con vista embebida vía `iframe` y botón de cierre `✕`, permitiendo navegar la página de grupos sin abandonar la sesión del usuario.
*   [x] **Política de Onboarding Persistente Obligatorio (`app.js`):** Ajustada la función `init()` para que cada vez que el usuario ingrese a la aplicación o recargue la página, si no ha completado el onboarding (`completed: false`), el modal se despliegue automáticamente. Una vez completado (`completed: true`), el modal no vuelve a aparecer.
